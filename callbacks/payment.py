...
async def send_payment(bot: Bot, user_amount: int, char_id: int, user_id: int, payment_service: PaymentService):
    pay_id = await payment_service.create_replenishment_operations(user_id, user_amount)
...
async def message_payment(message: Message, bot: Bot, payment_service: PaymentService):
...
async def call_payment(callback_query: CallbackQuery, bot: Bot, payment_service: PaymentService):
...
async def successful_payment(message: Message, payment_service: PaymentService, state: FSMContext):
    user_id = message.from_user.id
    total_amount = message.successful_payment.total_amount
    pay_id = int(message.successful_payment.invoice_payload)

    await state.clear()
    await payment_service.update_balance(user_id, pay_id)
    await payment_service.payment_replenishment_operations(pay_id, total_amount)
    await message.answer('Успешное пополнеие.', reply_markup=back_profile)



@router.callback_query(F.data.split('|')[0] == 'check_payment')
async def check_payment(callback_query: CallbackQuery, payment_service: PaymentService):
    pay_id = int(callback_query.data.split('|')[-1])

    if await payment_service.check_payment(pay_id):
        await callback_query.message.edit_text(
